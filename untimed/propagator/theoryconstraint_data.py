...
class LitUsage:
	...
	def _check(cls, lit, control):
		...
		if cls.lit2usage[lit] < 0:
			return
			#raise ValueError(f"Usage of lit {lit} is lower than zero!")
		
		if cls.lit2usage[lit] == 0:
			util.Count.add("watches removed")
			control.remove_watch(lit)

	@classmethod			
	def check(cls, lits, control):
		for lit in lits:
			try:
				cls._check(lit, control)
			except ValueError:
				print(lits)
				raise